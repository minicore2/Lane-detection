...
void leftimage(const sensor_msgs::ImageConstPtr& original_image)
{
	...
	final_grid = final_grid + roi;//LeftROI->roi
...
void rightimage(const sensor_msgs::ImageConstPtr& original_image)
{
	...
	final_grid = final_grid + roi;//RightROI->roi
...
void interpolate(int, void*)
{
    ...
    findContours(src_gray, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_NONE);//src_gray->src
    ...
    for (int i = 0; i < src_gray.rows; ++i)//src_gray->src
    {
        for (int j = 0; j < src_gray.cols; ++j)//src_gray->src
        {
            if ( src_gray.at<uchar>(i,j) > 0)//src_gray->src
            {
                //cout<<"r";
                ...
                Final_Grid.data.push_back(src_gray.at<uchar>(i,j));//src_gray->src
        }
    }   
