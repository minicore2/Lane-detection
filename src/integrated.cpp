...
const int lane_in_cm = 35;       //radius around lane for path planning
const int side_in_cm = 30;
const int base_lidar_dist = 10;   //distance from vehicle centre to lidar in cm(only in y-direction)
   ...
   for(i=0;i<num_values;i++)
   {
      ...
      if(x_coord > 0 && x_coord <= range)//limiting obstacle detection to only 10m
      {
         if(y_coord <= range && y_coord >= -range)
         {
            ...
            obs_map.at<uchar>(x_coord,y_coord+width/2) = 1;
         } 
      }
   }
  ...
  for(int i=0;i<l_height;i++)//adding lane data 
   {
      for(int j=0;j<l_width;j++)
      {
         ...
         { 
            ...
            obs_map.at<uchar>(i,j+width/2-l_width/2) = 2;
         }
      }
   }
}
...
void blowmap()
{
  ...
  //cout<<"hi"<<endl;
   ...
   for(i=0;i<height;i++)//creating blown map
   {
      for(j=0;j<width;j++)
      {
         if(obs_map.at<uchar>(i,j)==1)//blowing obstacle data
         {
            //cout<<"hey"<<endl;
         ...
         if(obs_map.at<uchar>(i,j)==2)//blowing lane data
         {
   ...
   for(i=0;i<height;i++)
   {
     ...
     {
       int i_ = height - i;
       grid_map.data[i*width+j] = obs_map.at<uchar>(i,j);
       local_map.at<uchar>(i,j) = grid_map.data[i_*width+j]*0.5*255;
...
int main(int argc, char **argv)
{
  ...
  map_pub=nh.advertise<nav_msgs::OccupancyGrid>("scan/local_map",1);
  ...
  { 
    ...
    final_grid = Mat::zeros(Size(occ_grid_width, occ_grid_height), CV_8UC1);
    obs_map = Mat::zeros(height,width,CV_8UC1);
    ros::spinOnce();
